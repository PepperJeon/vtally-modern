...
2. ~~**Hub-disconnected banner.** Cheap to add (one `it` visiting `/` with the hub socket down, asserting the existing `Alert`), currently zero coverage despite the component existing and being simple to break by accident during a full rewrite of `Layout`/`IndexPage`.~~ **Done — and the outcome is worth more than the recommendation was.** `hub-disconnected-banner.spec.ts` exists and cuts the connection for real via CDP. It also passed continuously while the banner was showing on *every* cold load of a healthy hub, because it asserts its "hub connected" precondition against a startup race that Cypress won by ~12ms and a plain browser lost by 45ms. The gap this list should carry forward is not "add a spec" but **a spec whose precondition is itself timing-dependent reports green from the wrong side of the bug**. Fixed in `useSocketInfo.ts`; the spec is now deterministic (12/12 `flake2.sh`, no variance). See `DECISIONS.md` → Known bugs — recently closed.
