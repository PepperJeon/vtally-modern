...
| `/flasher` (`FlasherPage`) | Default "no device connected" state only. | Effectively everything active: `flasher.settingsIni` and `flasher.program` emits are only exercised by `manual_flasher.spec.ts` (hardware-gated, never runs in CI). Progress dialog, step-by-step upload UI, success/error states — all untested in CI. `flasher.spec.ts` is a 14-line stub. |
